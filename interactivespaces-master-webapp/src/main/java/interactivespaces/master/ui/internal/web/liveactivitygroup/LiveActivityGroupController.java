 ...
 */
@Controller
public class LiveActivityGroupController extends
		BaseActiveSpaceMasterController {
	...
	public ModelAndView listActivities() {
		List<LiveActivityGroup> groups = Lists.newArrayList(activityRepository
	...
	public ModelAndView viewActivityGroup(@PathVariable String id) {
		...
		if (liveActivityGroup != null) {
			...
			mav.addObject("metadata", UiUtilities
					.getMetadataView(liveActivityGroup.getMetadata()));
			...
			List<Space> spaces = Lists.newArrayList(activityRepository
					.getSpacesByLiveActivityGroup(liveActivityGroup));
	...
	public @ResponseBody
	Map<String, ? extends Object> deactivateActivityGroup(
	...
	public @ResponseBody
	Map<String, ? extends Object> shutdownLiveActivityGroup(
			@PathVariable String id) {
		try {
	...
	public @ResponseBody
	Map<String, ? extends Object> forceShutdownLiveActivityLiveActivityGroup(
			@PathVariable String id) {
		try {
			uiControllerManager
					.forceShutdownLiveActivitiesLiveActivityGroup(id);
	...
	public @ResponseBody
	Map<String, ? extends Object> modifyLiveActivityZGroupMetadata(
		...
		if (Map.class.isAssignableFrom(metadataCommandObj.getClass())) {
			...
			return uiActivityManager.updateLiveActivityGroupMetadata(id,
					metadataCommand);
		} else {
			return JsonSupport
	...
	Map<String, ? extends Object> listAllGroupsJson(
		...
		try {
			...
			for (LiveActivityGroup group : activityRepository
					.getLiveActivityGroups(filterExpression)) {
				Map<String, Object> groupData = Maps.newHashMap();

				uiActivityManager.getBasicLiveActivityGroupJsonData(group,
						groupData);
