...
public class LiveActivityConfigEditForm extends BaseSpaceMasterController {
	...
	/**
	 * Create a configuration form with the values of the configuration
	 * parameters in it.
	 * 
	 * @param liveactivity
	...
	private ConfigurationForm newConfigurationForm(LiveActivity liveactivity) {
		...
		if (configuration != null) {
			List<ConfigurationParameter> configParameters = Lists
					.newArrayList(configuration.getParameters());

			// Want the parameters sorted by name.
			Collections.sort(configParameters,
					new Comparator<ConfigurationParameter>() {
						@Override
						public int compare(ConfigurationParameter o1,
								ConfigurationParameter o2) {
	 ...
	 */
	private boolean saveConfigurationForm(ConfigurationForm form,
