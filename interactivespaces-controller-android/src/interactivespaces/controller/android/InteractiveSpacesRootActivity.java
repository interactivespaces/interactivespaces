...
public class InteractiveSpacesRootActivity extends Activity {
  ...
  private void ensureUuid(SharedPreferences prefs) {
    String uuid = prefs.getString(AndroidConfigurationProvider.PREF_CONTROLLER_UUID, null);
    if (uuid == null) {
      uuid = UUID.randomUUID().toString();
      Log.i(AndroidLoggingProvider.BASE_LOG_NAME, String.format("No UUID, generated %s", uuid));
      SharedPreferences.Editor editor = prefs.edit();
      editor.putString(AndroidConfigurationProvider.PREF_CONTROLLER_UUID, uuid);
  ...
  public void updateView(Context context) {
    ...
    TextView interactivespacesVersionView =
        (TextView) findViewById(R.id.myInteractivespacesVersion);
    try {
      PackageManager pm = getPackageManager();
      PackageInfo pi = pm.getPackageInfo("interactivespaces.controller.android", 0);
    ...
    deviceSerialView.setText(Settings.Secure.getString(context.getContentResolver(),
    ...
    String localIpAddress = AndroidInteractiveSpacesEnvironment.getLocalIpAddress();
    ...
    controllerHostView.setText(prefs.getString(AndroidConfigurationProvider.PREF_CONTROLLER_HOST,
        "?required?"));

    TextView hostIdView = (TextView) findViewById(R.id.myHostId);
    hostIdView.setText(prefs.getString(AndroidConfigurationProvider.PREF_HOST_ID, "?required?"));

    TextView controllerUuidView = (TextView) findViewById(R.id.myControllerUuid);
    controllerUuidView.setText(prefs.getString(AndroidConfigurationProvider.PREF_CONTROLLER_UUID,
        "?required?"));

    TextView controllerNameView = (TextView) findViewById(R.id.myControllerName);
    controllerNameView.setText(prefs.getString(AndroidConfigurationProvider.PREF_CONTROLLER_NAME,
        "?required?"));
    ...
    controllerDescriptionView.setText(prefs.getString(
        AndroidConfigurationProvider.PREF_CONTROLLER_DESCRIPTION, "?required?"));

    TextView masterHostView = (TextView) findViewById(R.id.myMasterHost);
    masterHostView.setText(prefs.getString(AndroidConfigurationProvider.PREF_MASTER_HOST,
        "?required?"));

    TextView masterPortView = (TextView) findViewById(R.id.myMasterPort);
    masterPortView.setText(prefs.getString(AndroidConfigurationProvider.PREF_MASTER_PORT,
        "?required?"));
  }
