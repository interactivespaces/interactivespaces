...
public class InteractiveSpacesBootReceiver extends BroadcastReceiver {
  ...
  public void onReceive(Context context, Intent myIntent) {
    Log.i(AndroidLoggingProvider.BASE_LOG_NAME, "INTENT RECEIVED: " + myIntent.getAction());
    ...
    if ((myIntent.getAction()).equals("android.intent.action.BOOT_COMPLETED")) {
      ...
      Log.i(AndroidLoggingProvider.BASE_LOG_NAME, "onReceive: " + myIntent.getAction());

      SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(context);

      if (prefs.getBoolean("PREF_START_ON_BOOT", false)) {
        Log.i(AndroidLoggingProvider.BASE_LOG_NAME,
            "Autostarting Interactive Spaces controller on boot");
        AndroidInteractiveSpacesEnvironment.startInteractiveSpacesService(context);
      }
    }
  }
