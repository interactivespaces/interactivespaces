...
public class InteractiveSpacesService extends Service {
  ...
  public int onStartCommand(Intent intent, int flags, int startId) {
    ...
    try {
      initialProperties =
          AndroidConfigurationProvider.getInitialProperties(PreferenceManager
    ...
    Notification notification =
        new Notification(R.drawable.ic_launcher, "Starting up Interactive Spaces Controller",
    ...
    Log.i(AndroidLoggingProvider.BASE_LOG_NAME, "startForeground for Interactive Spaces Controller");

    isBootstrap = new InteractiveSpacesFrameworkAndroidBootstrap();
    isBootstrap.startup(initialProperties, getApplicationContext());
    Log.i(AndroidLoggingProvider.BASE_LOG_NAME, "starting Interactive Spaces container");

    PowerManager pm = (PowerManager) getSystemService(Context.POWER_SERVICE);
    wakelock = pm.newWakeLock(PowerManager.FULL_WAKE_LOCK, "InteractiveSpacesRootActivity");
    wakelock.acquire();
  ...
  public void onDestroy() {
    Log.i(AndroidLoggingProvider.BASE_LOG_NAME, "InteractiveSpacesService got onDestroy()");
