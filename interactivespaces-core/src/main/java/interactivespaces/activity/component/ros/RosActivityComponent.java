...
public class RosActivityComponent extends BaseActivityComponent {
	...
	/**
	 * Configuration property for specifying the Interactive Spaces ROS node
	...
	public void configureComponent(Configuration configuration,
		...
		rosEnvironment = componentContext.getActivity().getSpaceEnvironment()
				.getValue("environment.ros");

		rosNodeName = configuration
				.getRequiredPropertyString(CONFIGURATION_ACTIVITY_ROS_NODE_NAME);

		nodeConfiguration = rosEnvironment
	...
	public void startupComponent() {
		...
		getComponentContext()
				...
				.getLog()
				.info(String.format(
						"Start up ROS activity component with ROS node name %s",
						node.getName()));
	}
