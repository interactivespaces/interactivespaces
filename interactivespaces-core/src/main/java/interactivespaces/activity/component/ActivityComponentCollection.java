...
public class ActivityComponentCollection {
	...
	/**
	 * Configure all the components in the in the collection in dependency
	 * order.
	 */
	public void configureComponents(Configuration configuration,
		...
		for (ActivityComponent component : components) {
			...
			resolver.addNodeDependencies(component.getName(),
		...
		for (ActivityComponent component : components) {
			if (!component.isComponentRunning()) {
				...
				component
						...
						.getLog()
						.error(String
								.format("Activity component %s not running when expected",
										component.getName()));
			}
		}
	...
	public <T extends ActivityComponent> T getActivityComponent(String name) {
		...
		throw new InteractiveSpacesException(String.format(
				"No activity component with name %s", name));
	}
