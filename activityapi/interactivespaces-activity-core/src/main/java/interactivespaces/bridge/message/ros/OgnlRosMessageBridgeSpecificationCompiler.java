...
public class OgnlRosMessageBridgeSpecificationCompiler implements
  ...
  public <SourceType, DestinationType> RosMessageBridgeSpecification<SourceType, DestinationType>
    ...
    if (lines.length < 2) {
      throw new InteractiveSpacesException("Not enough lines in the bridge specification");
    }

    String[] sources = lines[0].split(":");
    if (sources.length != 2) {
      throw new InteractiveSpacesException(String.format("Illegal message bridge source %s",
          lines[0]));
    }

    String[] destinations = lines[1].split(":");
    if (destinations.length != 2) {
      throw new InteractiveSpacesException(String.format("Illegal message bridge destination %s",
          lines[1]));
    }
