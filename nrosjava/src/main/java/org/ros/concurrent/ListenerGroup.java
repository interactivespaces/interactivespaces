...
public class ListenerGroup<T> {
	...
	public EventDispatcher<T> add(T listener, int queueCapacity) {
		EventDispatcher<T> eventDispatcher = new EventDispatcher<T>(listener,
				queueCapacity);
	 ...
	 * <p>
	 * Each {@link SignalRunnable} is executed in a separate thread. In the
	 * event that the {@link SignalRunnable} is be dropped from the
	 ...
	 */
	public boolean signal(final SignalRunnable<T> signalRunnable, long timeout,
			TimeUnit unit) throws InterruptedException {
		Collection<EventDispatcher<T>> copy = Lists
				.newArrayList(eventDispatchers);
