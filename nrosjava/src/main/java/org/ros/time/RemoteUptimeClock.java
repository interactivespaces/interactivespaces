...
public class RemoteUptimeClock {
	...
	private static final Log log = RosLogFactory
			.getLog(RemoteUptimeClock.class);
	 ...
	 * will never change (i.e. new measurements have no effect on estimates) and
	 * 1 indicates that previous estimates have no effect on changes to the
	 * current estimate.
	 */
	...
	/**
	 * Remote uptime is tracked as a pair of values: our previous measurement
	 * and our prediction based on estimated drift.
	 */
	 ...
	 * that will never be corrected for. The {@link #errorReductionCoefficient}
	 * is an additional term for removing this error.
	 */
	...
	private final class UptimeCalculationResult {
		...
		public UptimeCalculationResult(double newLocalUptime,
				double newRemoteUptime, double latency) {
	 ...
	 * symmetrical. The larger the latency, the larger the potential error in
	 * our estimate of local uptime at the measured remote uptime.
	 * 
	 * <p>
	 * To reduce the effect of measurements with higher uncertainty, we filter
	 * out measurements with latencies that exceed the specified percentile
	...
	private final class LatencyOutlierFilter {
		...
		/**
		 * @param latency
		 * @return {@code true} if the provided latency is outside the
		 *         configured percentile, {@code false} otherwise
		 */
	 ...
	 *            the sensitivity to drift adjustments, must be in the range [0,
	 *            1]
	 * @param errorReductionCoefficientSensitivity
	 *            the sensitivity to error reduction coefficient adjustments,
	...
	RemoteUptimeClock(LocalUptimeProvider localUptimeProvider,
			...
			int latencyOutlierFilterSampleSize,
			double latencyOutlierFilterThreshold) {
		Preconditions.checkArgument(driftSensitivity >= 0
				&& driftSensitivity <= 1);
		...
		latencyOutlierFilter = new LatencyOutlierFilter(
				latencyOutlierFilterSampleSize, latencyOutlierFilterThreshold);
	 ...
	 * general, choosing a sample size around 10 and a delay that is large
	 * enough to include more than 100 uptime ticks will give reasonable
	 * results.
	 * 
	 * @param sampleSize
	...
	public void calibrate(int sampleSize, double samplingDelayMillis) {
		...
		for (int i = 0; i < sampleSize; i++) {
			...
			if (i > 0) {
				...
				double remoteUptimeDelta = result.newRemoteUptime
						- measuredRemoteUptime;
		...
		log.info(String.format(
				"Calibration complete. Drift: %.4g, Offset: %.4f s", drift,
				offset));
	}
	 ...
	 *            the delta between the two local uptimes that correspond to the
	 *            two remote uptimes used to determine {@code remoteUptimeDelta}
	 * @param remoteUptimeDelta
	 *            the delta between the two remote uptimes that correspond to
	 *            the two local uptimes used to determine
	 *            {@code localUptimeDelta}
	 * @return the calculated drift
	 */
	private double calculateDrift(double localUptimeDelta,
	 ...
	 * 
	 * <p>
	 * This will update internal estimates of drift and error. Ideally, it
	 * should be called periodically with a consistent time interval between
	 * updates (e.g. 10 seconds).
	 */
	public void update() {
		UptimeCalculationResult result = calculateNewUptime(callable);
		...
		if (latencyOutlierFilter.add(latency)) {
			log.warn(String
					.format("Measurement latency marked as outlier. Latency: %.4f s, Median: %.4f s",
		...
		if (log.isDebugEnabled()) {
			log.debug(String.format(
					"localUptimeDelta: %.4g, remoteUptimeDelta: %.4g",
					localUptimeDelta, remoteUptimeDelta));
		}

		double newDrift = driftSensitivity
				* (localUptimeDelta / remoteUptimeDelta)
		...
		double newCombinedDriftAndError = localUptimeDelta
				/ (nextPredictedRemoteUptime - newPredictedRemoteUptime);
		...
		log.info(String.format(
				...
				latency, deltaRatio, newDrift, newErrorReductionCoefficient,
				error));
	 ...
	 * been adjusted to compensate for latency while retrieving the remote
	 * uptime.
	 * 
	 * @param callable
	...
	private UptimeCalculationResult calculateNewUptime(Callable<Double> callable) {
		...
		return new UptimeCalculationResult(newLocalUptime, newRemoteUptime,
				latency);
	}

	/**
	 * Returns the estimated local uptime in seconds for the given remote
	 * uptime.
	 * 
	 * @param remoteUptime
	 *            the remote uptime to convert to local uptime
	 * @return the estimated local uptime in seconds at the provided remote
	 *         uptime
	 */
	public double toLocalUptime(double remoteUptime) {
		double localOffset = (drift + errorReductionCoefficient)
				* (remoteUptime - predictedRemoteUptime);
