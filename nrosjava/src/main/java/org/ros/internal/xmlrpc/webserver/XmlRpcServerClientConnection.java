...
public class XmlRpcServerClientConnection implements ServerStreamConnection {
	...
	public void process() {
		...
		try {
			...
			handler.getWebServer().getLog()
					.error("Error during XMLRPC server request handling", e);
		}
	}
	...
	private XmlRpcServerClientRequestData getRequestConfig() {
		...
		XmlRpcHttpServerConfig serverConfig = (XmlRpcHttpServerConfig) xmlRpcServer
				.getConfig();
		requestData.setBasicEncoding(serverConfig.getBasicEncoding());
		requestData.setContentLengthOptional(serverConfig
				.isContentLengthOptional());
		requestData.setEnabledForExtensions(serverConfig
				.isEnabledForExtensions());
		requestData.setEnabledForExceptions(serverConfig
	...
	public ChannelBuffer getChannelBuffer() {
		try {
			...
			throw new RosRuntimeException(
					"Could not flush XMLRPC server connection output stream", e);
		} finally {
			if (outputStream != null) {
				try {
					outputStream.close();
					...
					throw new RosRuntimeException(
							"Could not close XMLRPC server connection output stream",
							e);
				}
			}
		}
