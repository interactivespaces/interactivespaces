	...
	public <T> Publisher<T> newOrExisting(TopicDeclaration topicDeclaration,
		...
		synchronized (mutex) {
			if (topicParticipantManager.hasPublisher(topicName)) {
				return (DefaultPublisher<T>) topicParticipantManager
						.getPublisher(topicName);
			} else {
				DefaultPublisher<T> publisher = new DefaultPublisher<T>(
						nodeIdentifier, topicDeclaration, messageSerializer,
				...
				publisher.addListener(new DefaultPublisherListener<T>() {
					...
					public void onNewSubscriber(Publisher<T> publisher,
						...
						topicParticipantManager.addPublisherConnection(
