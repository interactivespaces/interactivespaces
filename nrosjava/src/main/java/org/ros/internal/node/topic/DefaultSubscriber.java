...
public class DefaultSubscriber<T> extends DefaultTopicParticipant implements
	...
	/**
	 * This latch will tell us when there has been proper unregistration from
	 * the master and all listeners have been signaled of shutdown.
	 */
	...
	public static <S> DefaultSubscriber<S> newDefault(
			NodeIdentifier nodeIdentifier, TopicDeclaration description,
	...
	private DefaultSubscriber(NodeIdentifier nodeIdentifier,
		...
		mutex = new Object();
		SubscriberHandshakeHandler<T> subscriberHandshakeHandler = new SubscriberHandshakeHandler<T>(
				toDeclaration().toConnectionHeader(), incomingMessageQueue,
				executorService);
		tcpClientManager.addNamedChannelHandler(subscriberHandshakeHandler);
		subscriberListeners = new ListenerGroup<SubscriberListener<T>>(
				executorService);
	...
	public SubscriberIdentifier toIdentifier() {
		return new SubscriberIdentifier(nodeIdentifier, getTopicDeclaration()
				.getIdentifier());
	}
	...
	public void updatePublishers(
		...
		for (final PublisherIdentifier publisherIdentifier : publisherIdentifiers) {
			executorService.execute(new UpdatePublisherRunnable<T>(this,
					nodeIdentifier, publisherIdentifier));
		}
	}
	...
	private void signalOnShutdown(long timeout, TimeUnit unit) {
		final Subscriber<T> subscriber = this;
		try {
			subscriberListeners.signal(
					new SignalRunnable<SubscriberListener<T>>() {
						@Override
						public void run(SubscriberListener<T> listener) {
