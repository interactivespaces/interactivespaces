...
public class DefaultPublisher<T> extends DefaultTopicParticipant implements
	...
	public DefaultPublisher(NodeIdentifier nodeIdentifier,
			TopicDeclaration topicDeclaration, MessageSerializer<T> serializer,
			MessageFactory messageFactory,
			ScheduledExecutorService executorService) {
		super(topicDeclaration);
		this.nodeIdentifier = nodeIdentifier;
		this.messageFactory = messageFactory;
		outgoingMessageQueue = new OutgoingMessageQueue<T>(serializer,
				executorService);
		...
		listeners.add(new DefaultPublisherListener<T>() {
			...
			public void onMasterRegistrationFailure(Publisher<T> registrant) {
				log.info("Publisher registration failed: "
						+ DefaultPublisher.this);
			}
			...
			public void onMasterUnregistrationFailure(Publisher<T> registrant) {
				log.info("Publisher unregistration failed: "
						+ DefaultPublisher.this);
			}
		});
	}
	...
	public PublisherIdentifier getIdentifier() {
		return new PublisherIdentifier(nodeIdentifier, getTopicDeclaration()
				.getIdentifier());
	}

	public PublisherDeclaration toDeclaration() {
		return PublisherDeclaration.newFromNodeIdentifier(nodeIdentifier,
				getTopicDeclaration());
	}
	...
	@Override
	public T newMessage() {
		return messageFactory.newFromType(getTopicDeclaration()
				.getMessageType());
	}
	 ...
	 * header for this publisher to send and also updates the connection state
	 * of this publisher.
	 * 
	...
	public ChannelBuffer finishHandshake(ConnectionHeader incomingHeader) {
		...
		String incomingType = incomingHeader
				.getField(ConnectionHeaderFields.TYPE);
		String expectedType = topicDefinitionHeader
				.getField(ConnectionHeaderFields.TYPE);
		boolean messageTypeMatches = incomingType.equals(expectedType)
				|| incomingType.equals(Subscriber.TOPIC_MESSAGE_TYPE_WILDCARD);
		Preconditions.checkState(messageTypeMatches, "Unexpected message type "
				+ incomingType + " != " + expectedType);
		String incomingChecksum = incomingHeader
				.getField(ConnectionHeaderFields.MD5_CHECKSUM);
		String expectedChecksum = topicDefinitionHeader
				.getField(ConnectionHeaderFields.MD5_CHECKSUM);
		boolean checksumMatches = incomingChecksum.equals(expectedChecksum)
				|| incomingChecksum
						.equals(Subscriber.TOPIC_MESSAGE_TYPE_WILDCARD);
		Preconditions.checkState(checksumMatches, "Unexpected message MD5 "
				+ incomingChecksum + " != " + expectedChecksum);
		ConnectionHeader outgoingConnectionHeader = toDeclaration()
				.toConnectionHeader();
		// TODO(damonkohler): Force latch mode to be consistent throughout the
		// life
		// of the publisher.
		outgoingConnectionHeader.addField(ConnectionHeaderFields.LATCHING,
				getLatchMode() ? "1" : "0");
	...
	/**
	 * Signal all {@link PublisherListener}s that the {@link Publisher} has a
	 * new {@link Subscriber}.
	 * <p>
	 * Each listener is called in a separate thread.
	 * 
