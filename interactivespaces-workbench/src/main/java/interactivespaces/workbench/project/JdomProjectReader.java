...
public class JdomProjectReader implements ProjectReader {
	...
	public Project readDescription(File projectFile) {
		...
		try {
			...
			throw new InteractiveSpacesException(
					"Unable to read project description", e);
		} finally {
			if (inputStream != null) {
				try {
					inputStream.close();
	 ...
	 *            any errors found in the metadata
	 */
	private void getResources(Project project, Element rootElement,
			List<String> errors) {
		Element resourcesElement = rootElement.getChild("resources");

		if (resourcesElement != null) {
			@SuppressWarnings("unchecked")
			List<Element> resourceElements = resourcesElement
	 ...
	 * @return the dependency found in the element
	 */
	private ProjectResource getResource(Element resourceElement,
			List<String> errors) {
		...
		String destDir = resourceElement
				.getAttributeValue("destinationDirectory");
		String destFile = resourceElement.getAttributeValue("destinationFile");

		if (sourceFile == null && sourceDir == null) {
			addedErrors = true;
			errors.add("Resource has no destination");
		}
