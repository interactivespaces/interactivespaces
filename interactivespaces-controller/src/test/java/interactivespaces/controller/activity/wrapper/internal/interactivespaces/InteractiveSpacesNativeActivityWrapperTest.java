...
public class InteractiveSpacesNativeActivityWrapperTest {
	...
	public void setup() {
		...
		when(activityFilesystem.getInstallDirectory()).thenReturn(
				activityInstallFolder);

		configuration = mock(Configuration.class);
		bundleLoader = mock(LiveActivityBundleLoader.class);

		wrapper = new InteractiveSpacesNativeActivityWrapper(liveActivity,
	...
	public void testSingleNew() throws Exception {
		...
		when(
				bundleLoader.getBundleClass(executableFile1, bundleName,
						bundleVersion, className)).thenReturn(
		...
		verify(bundleLoader, times(1)).getBundleClass(executableFile1,
				bundleName, bundleVersion, className);
	}
	...
	public void testTwoNew() throws Exception {
		...
		verify(bundleLoader, times(2)).getBundleClass(executableFile1,
				bundleName, bundleVersion, className);
	}
	...
	public void testCofigChangeExecutable() throws Exception {
		...
		when(
				bundleLoader.getBundleClass(executableFile2, bundleName,
						bundleVersion, className)).thenReturn(
		...
		verify(bundleLoader, times(1)).getBundleClass(executableFile1,
				bundleName, bundleVersion, className);
		verify(bundleLoader, times(1)).getBundleClass(executableFile2,
				bundleName, bundleVersion, className);
	}
	...
	public void testCofigChangeClassName() throws Exception {
		...
		when(
				bundleLoader.getBundleClass(executableFile1, bundleName,
						bundleVersion, className1)).thenReturn(
				expectedActivityClass1);
		when(
				bundleLoader.getBundleClass(executableFile1, bundleName,
						bundleVersion, className2)).thenReturn(
				expectedActivityClass2);
		...
		verify(bundleLoader, times(1)).getBundleClass(executableFile1,
				bundleName, bundleVersion, className1);
		verify(bundleLoader, times(1)).getBundleClass(executableFile1,
				bundleName, bundleVersion, className2);
	}
	...
	public void testCofigChangeBundleNameVersion() throws Exception {
		...
		Class expectedActivityClass2 = Activity2.class;
		when(
				bundleLoader.getBundleClass(executableFile1, bundleName1,
						bundleVersion1, className)).thenReturn(
				expectedActivityClass1);
		when(
				bundleLoader.getBundleClass(executableFile1, bundleName2,
						bundleVersion2, className)).thenReturn(
				expectedActivityClass2);
		...
		verify(bundleLoader, times(1)).getBundleClass(executableFile1,
				bundleName1, bundleVersion1, className);
		verify(bundleLoader, times(1)).getBundleClass(executableFile1,
				bundleName2, bundleVersion2, className);
	}
