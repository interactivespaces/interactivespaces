...
public class ActiveControllerActivity implements ActivityControl {
	...
	public void updateConfiguration(Map<String, Object> update) {
		...
		synchronized (instanceLock) {
			if (instance != null
					&& instance.getActivityStatus().getState().isRunning()) {
	...
	public void startup() {
		synchronized (instanceLock) {
			if (instance == null) {
				...
				controller.initializeInstance(installedActivity,
						activityFilesystem, instance, configuration,
						activityWrapper.newExecutionContext());
				instance.startup();
			} else {
				throw new InteractiveSpacesException(
						String.format(
								"Attempt to start activity %s which is already started",
								uuid));
			}
		}
	}
		...
		synchronized (instanceLock) {
			if (instance != null) {
				...
				throw new InteractiveSpacesException(String.format(
						"Attempt to activate activity %s which is not started",
						uuid));
			}
		}
	}
	...
	public void deactivate() {
		synchronized (instanceLock) {
			if (instance != null) {
				...
				throw new InteractiveSpacesException(
						String.format(
								"Attempt to deactivate activity %s which is not started",
								uuid));
			}
		}
	}
	 ...
	 * 
	 * <p>
	 * This will be the last known state of the activity. The activity itself
	 * will be sampled.
	 * 
