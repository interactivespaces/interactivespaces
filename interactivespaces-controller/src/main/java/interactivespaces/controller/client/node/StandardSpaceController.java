...
public class StandardSpaceController implements SpaceController,
	...
	private ActiveControllerActivity newLiveActivityFromRepository(String uuid) {
		...
		if (liveActivity != null) {
			...
			LiveActivityConfiguration activityConfiguration = configurationManager
		...
		try {
			...
			case RUNNING:
				...
				spaceEnvironment
						.getLog()
						.warn(String
								.format("Attempt to startup live activity %s which was running, sending RUNNINg",
										activity.getUuid()));
	...
	private void attemptActivityActivate(ActiveControllerActivity activity) {
		...
		case ACTIVE:
			...
			spaceEnvironment
					.getLog()
					.warn(String
							.format("Attempt to activate live activity %s which was activated, sending ACTIVE",
									activity.getUuid()));
			publishActivityStatus(activity.getUuid(),
					LIVE_ACTIVITY_ACTIVE_STATUS);
			break;
