...
public class PythonActivityScriptWrapper implements ActivityScriptWrapper {
  ...
  public Activity newInstance() {
    try {
      ...
      if (cls == null)
        throw new InteractiveSpacesException(String.format("No callable class named %s in %s",
            objectName, script));
      ...
      if (o == Py.NoConversion)
        throw new InteractiveSpacesException(String.format(
            "The value with name %s must implement %s", objectName, Activity.class.getName()));

      return (Activity) o;
    } catch (PyException e) {
      e.printStackTrace();
      throw new InteractiveSpacesException("Could not create Jython Interactive Spaces activity ",
          e);
    }
  }
