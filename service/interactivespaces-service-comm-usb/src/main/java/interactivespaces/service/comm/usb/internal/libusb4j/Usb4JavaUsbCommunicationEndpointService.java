...
public class Usb4JavaUsbCommunicationEndpointService implements
	...
	public void startup() {
		try {
			...
			throw new InteractiveSpacesException(
					"Could not obtain the USB Root Hub", e);
		}
	}
	...
	public UsbCommunicationEndpoint newEndpoint(String vendor, String product) {
		UsbDevice device = findDevice(Integer.valueOf(vendor, 16),
				Integer.valueOf(product, 16), rootHub);
	...
	private UsbDevice findDevice(int vendor, int product, UsbDevice currDevice) {
		if (currDevice.isUsbHub()) {
			...
			List<UsbDevice> attachedUsbDevices = (List<UsbDevice>) hub
			...
			if ((desc.idVendor() & 0xffff) == vendor
					&& (desc.idProduct() & 0xffff) == product) {
				return currDevice;
			}
		}
