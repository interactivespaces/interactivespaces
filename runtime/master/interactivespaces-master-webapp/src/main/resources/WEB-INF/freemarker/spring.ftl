...
<#macro formMultiSelect path options attributes="">
    <@bind path/>
    <#if status.value??><#assign selected_values = status.value?split(",")><#else><#assign selected_values = []></#if>
    <select multiple="multiple" id="${status.expression}" name="${status.expression}" ${attributes}>
        <#list options?keys as value>
        <option value="${value?html}"<#if selected_values?seq_contains(value)> selected="selected"</#if>>${options[value]?html}</option>
        </#list>
    </select>
</#macro>
