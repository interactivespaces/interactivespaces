...
/**
 * Extension of {@link XmlRpcServer} with support for reading requests from a
 * stream and writing the response to another stream.
 */
public abstract class XmlRpcStreamServer extends XmlRpcServer implements XmlRpcStreamRequestProcessor {
  ...
  /**
   * This method allows to convert the error into another error. For example,
   * this may be an error, which could be deserialized by the client.
   */
   ...
   *          The requests size.
   */
  protected OutputStream getOutputStream(XmlRpcStreamRequestConfig pConfig, ServerStreamConnection pConnection,
