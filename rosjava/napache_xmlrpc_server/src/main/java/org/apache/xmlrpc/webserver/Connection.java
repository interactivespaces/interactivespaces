...
/**
 * Handler for a single clients connection. This implementation is able to do
 * HTTP keepalive. In other words, it can serve multiple requests via a single,
 * physical connection.
 */
public class Connection implements ThreadPool.InterruptableTask, ServerStreamConnection {
  ...
  /**
   * Returns the US-ASCII encoded byte representation of text for HTTP use (as
   * per section 2.2 of RFC 2068).
   */
   ...
   *
   * @param pWebServer
   *          The webserver maintaining this connection.
   * @param pServer
   *          The server being used to execute requests.
   * @param pSocket
   *          The server socket to handle; the <code>Connection</code> is
   *          responsible for closing this socket.
   * @throws IOException
   */
  public Connection(WebServer pWebServer, XmlRpcStreamServer pServer, Socket pSocket) throws IOException {
    ...
    input = new BufferedInputStream(socket.getInputStream()) {
      /**
       * It may happen, that the XML parser invokes close(). Closing the input
       * stream must not occur, because that would close the whole socket. So we
       * suppress it.
       */
      @Override
  ...
  /**
   * Returns the connections request configuration by merging the HTTP request
    ...
    do {
      ...
      if (line != null) {
        ...
        if (lineLower.startsWith("content-length:")) {
          ...
          boolean pKeepAlive = serverConfig.isKeepAliveEnabled() && lineLower.indexOf("keep-alive") > -1;
   ...
   *          The request data.
   * @param pBuffer
   *          The {@link ByteArrayOutputStream} holding the response.
   * @throws IOException
  ...
  public void writeErrorHeader(RequestData pData, Throwable pError, int pContentLength) throws IOException {
    if (pError instanceof BadRequestException) {
      ...
      final byte[] content =
          toHTTPBytes("Method " + pData.getMethod() + " requires a " + "valid user name and password.\r\n");
      output.write(toHTTPBytes(pData.getHttpVersion()));
  ...
  public OutputStream newOutputStream() throws IOException {
    ...
    useContentLength =
        !requestData.isEnabledForExtensions() || !((XmlRpcHttpRequestConfig) requestData).isContentLengthOptional();
    if (useContentLength) {
