...
import org.ros.internal.transport.tcp.TcpClientConnectionListener;
...
public class DefaultSubscriber<T> extends DefaultTopic implements Subscriber<T> {
	...
	public static <S> DefaultSubscriber<S> newDefault(
			NodeIdentifier nodeIdentifier, TopicDefinition description,
			ScheduledExecutorService executorService,
			MessageDeserializer<S> deserializer) {
		return new DefaultSubscriber<S>(nodeIdentifier, description,
	...
	private DefaultSubscriber(NodeIdentifier nodeIdentifier,
		...
		incomingMessageQueue = new IncomingMessageQueue<T>(deserializer,
				executorService);
		knownPublishers = Sets.newHashSet();
		tcpClientConnectionManager = new TcpClientConnectionManager(
				executorService);
		subscriberListeners = new ListenerCollection<SubscriberListener<T>>(
				executorService);
		subscriberListeners.add(new DefaultSubscriberListener<T>() {
			...
			public void onMasterRegistrationFailure(Subscriber<T> registrant) {
				log.info("Subscriber registration failed: "
						+ DefaultSubscriber.this);
			}
			...
			public void onMasterUnregistrationFailure(Subscriber<T> registrant) {
				log.info("Subscriber unregistration failed: "
						+ DefaultSubscriber.this);
			}
		});
	}
	...
	public SubscriberIdentifier toIdentifier() {
		return new SubscriberIdentifier(nodeIdentifier, getTopicDefinition()
				.toIdentifier());
	}
	...
	public synchronized void addPublisher(
		...
		TcpClientConnectionListener listener = new TcpClientConnectionListener() {

			@Override
			public void onTcpClientConnectionMade() {
				knownPublishers.add(publisherIdentifier);
				signalOnNewPublisher();
			}

			@Override
			public void onTcpClientConnectionLost() {
				knownPublishers.remove(publisherIdentifier);
			}
		};
		tcpClientConnectionManager.connect(toString(), address, listener,
				new SubscriberHandshakeHandler<T>(toDefinition().toHeader(),
						incomingMessageQueue), "SubscriberHandshakeHandler");
	}
	 ...
	 * 
	 * @param publishers
	 *            {@link List} of {@link PublisherIdentifier}s for the
	 *            subscribed topic
	 */
	public void updatePublishers(Collection<PublisherIdentifier> publishers) {
		for (final PublisherIdentifier publisher : publishers) {
			executorService.execute(new UpdatePublisherRunnable<T>(this,
					this.nodeIdentifier, publisher));
		}
	}
	...
	private void signalShutdown() {
		...
		try {
			subscriberListeners
					.signal(new SignalRunnable<SubscriberListener<T>>() {
						@Override
						public void run(SubscriberListener<T> listener) {
