...
public class RosRemoteMasterServer implements RemoteMasterServer {
  ...
  public void startup() {
    ...
    masterTopicSubscriber =
        node.newSubscriber(RemoteMasterServerConstants.MASTER_SERVER_TOPIC_NAME,
            RemoteMasterServerConstants.MASTER_SERVER_TOPIC_MESSAGE_TYPE);
    masterTopicSubscriber.addMessageListener(new MessageListener<MasterServerData>() {
      @Override
    ...
    controllerDescriptionDeserializer =
  ...
  private void handleMasterServerData(MasterServerData data) {
    ...
    switch (data.getDataType()) {
      ...
      default:
        log.error(String.format("Unknown MasterServerData data type %d", data.getDataType()));
    }
  }
