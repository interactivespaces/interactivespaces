...
public class BasicUiControllerManager implements UiControllerManager {
	...
	public UiControllerManager connectToAllControllers() {
		for (SpaceController controller : controllerRepository
			...
			try {
				...
				spaceEnvironment.getLog().error(
						String.format(
								"Unable to connect to controller %s (%s)",
	...
	public UiControllerManager disconnectFromAllControllers() {
		for (SpaceController controller : controllerRepository
			...
			try {
				...
				spaceEnvironment.getLog().error(
						String.format(
								"Unable to disconnect to controller %s (%s)",
								controller.getUuid(), controller.getName()), e);
			}
		}
	...
	public UiControllerManager statusFromAllControllers() {
		for (SpaceController controller : controllerRepository
			...
			try {
				...
				spaceEnvironment.getLog().error(
						String.format(
								"Unable to get the status from controller %s (%s)",
								controller.getUuid(), controller.getName()), e);
			}
		}
			...
			if (controller != null) {
				try {
					...
					spaceEnvironment
							.getLog()
							.error(String.format(
									"Unable to shut down controller %s (%s)",
			...
			if (controller != null) {
				try {
					...
					spaceEnvironment
							.getLog()
							.error(String
									.format("Unable to disconnect from controller %s (%s)",
											controller.getUuid(),
											controller.getName()),
									e);
				}
	...
	public UiControllerManager shutdownAllControllers() {
		for (SpaceController controller : controllerRepository
			...
			try {
				...
				spaceEnvironment.getLog().error(
						String.format("Unable to shut down controller %s (%s)",
								controller.getUuid(), controller.getName()), e);
			}
		}
		...
		for (SpaceController controller : controllerRepository
			...
			try {
				...
				spaceEnvironment
						.getLog()
						.error(String.format(
								"Unable to shut down all live activities from controller %s (%s)",
								controller.getUuid(), controller.getName()), e);
			}
		}
	...
	public UiControllerManager deployAllControllerActivityInstances(String id) {
		...
		for (LiveActivity liveActivity : activityRepository
				.getLiveActivitiesByController(controller)) {
	...
	@Override
	public UiControllerManager forceShutdownLiveActivitiesLiveActivityGroup(
			String id) {
		LiveActivityGroup group = activityRepository
				.getLiveActivityGroupById(id);
		if (group == null) {
	 ...
	 *            IDs of all live activities which have had their status
	 *            requested so far
	 */
	...
	@Override
	public List<UiLiveActivity> getAllUiLiveActivitiesByController(
			SpaceController controller) {
		return getUiLiveActivities(activityRepository
				.getLiveActivitiesByController(controller));
	}
	...
	@Override
	public List<UiLiveActivity> getUiLiveActivities(
