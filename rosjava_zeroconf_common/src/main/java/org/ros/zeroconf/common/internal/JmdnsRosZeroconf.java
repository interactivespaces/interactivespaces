...
public class JmdnsRosZeroconf implements RosZeroconf {
  ...
  public void startup() {
    try {
      jmdns = JmDNS.create();
      jmdns.addServiceListener(ZEROCONF_TYPE_ROS_MASTER, listener = new ServiceListener() {
      ...
      throw new RosRuntimeException("Unable to start up ROS Zeroconf services", e);
    }
  }
  ...
  public void shutdown() {
    try {
      ...
      throw new RosRuntimeException("Unable to shut ROS Zeroconf services down", e);
    }
  }
  ...
  private void signalRemoveMaster(ZeroconfRosMasterInfo masterInfo) {
    if (rosEnvironment != null) {
      rosEnvironment.getLog().info(
          String.format("Zeroconf unregistering ROS Master %s", masterInfo));
    }
  ...
  private ServiceInfo getServiceInfo(ZeroconfRosMasterInfo masterInfo) {
    ...
    return ServiceInfo.create(ZEROCONF_TYPE_ROS_MASTER, masterInfo.getName(), masterInfo.getType(),
  ...
  private ZeroconfRosMasterInfo getMasterInfo(ServiceInfo info) {
    ...
    ZeroconfRosMasterInfo masterInfo =
        new ZeroconfRosMasterInfo(info.getName(), info.getSubtype(),
            info.getPropertyString("protocols"), hostName, info.getPort(), info.getPriority(),
            info.getWeight());
